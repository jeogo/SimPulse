...
class PortFilter:
    ...
    def _get_zte_port_priority(self, port: str) -> int:
        """Get priority score for ZTE ports (higher = better)"""
        try:
            ...
            # Check for ZTE with VID:PID=19D2:0016
            if '19d2:0016' in hwid and 'zte' in desc:
                # ZTE NMEA Device often supports USSD (highest priority for USSD)
                if 'nmea' in desc:
                    logger.info(f"Found ZTE NMEA Device (USSD capable): {port} - {desc}")
                    return 100
                # ZTE Diagnostics Interface should be good but may not respond
                if 'diagnostic' in desc:
                    logger.info(f"Found ZTE Diagnostics Interface: {port} - {desc}")
                    return 95
                # ZTE Proprietary USB Modem is lower priority (often no USSD support)
                if 'proprietary' in desc and 'modem' in desc:
                    logger.info(f"Found ZTE Proprietary Modem: {port} - {desc}")
                    return 80
                # Other ZTE ports get medium priority
                return 70
            
            # Non-ZTE ports get lower priority
            return 50
            
        except Exception as e:
            logger.debug(f"Could not get ZTE priority for port {port}: {e}")
            return 0
    ...
    def _group_ports_by_imei(self, valid_ports: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # Select the BEST port for each modem (prefer ZTE diagnostic ports)
            result = []
            for imei, ports in imei_groups.items():
                # Sort ports to choose the BEST one (ZTE diagnostic ports first)
                ports.sort(key=lambda x: (
                    self._get_zte_port_priority(x.get('port', '')),  # Prefer ZTE ports
    ...
    def _is_diagnostic_capable(self, port: str) -> bool:
        ...
        try:
            # First check if this is a ZTE diagnostic port (priority check)
            ports = list(serial.tools.list_ports.comports())
            port_info = None
            ...
            if port_info:
                ...
                # PRIORITY: ZTE ports with VID:PID=19D2:0016
                if '19d2:0016' in hwid and 'zte' in desc:
                    # ZTE NMEA Device often supports USSD (highest priority)
                    if 'nmea' in desc:
                        logger.info(f"ZTE NMEA Device detected (USSD capable): {port} - {desc}")
                        return True
                    # ZTE Diagnostics Interface should be good but may not respond
                    if 'diagnostic' in desc:
                        logger.info(f"ZTE Diagnostics Interface detected: {port} - {desc}")
                        return True
                    # ZTE Proprietary USB Modem lower priority for USSD
                    if 'proprietary' in desc and 'modem' in desc:
                        logger.info(f"ZTE Proprietary Modem detected: {port} - {desc}")
                        return True
